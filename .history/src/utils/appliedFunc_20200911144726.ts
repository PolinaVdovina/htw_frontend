...
export const resizeList = (imgFileList: FileList | undefined, maxWidth/*, onload*/) => {
    let canvas = document.createElement('canvas');
    let fileList: Array<File> = new Array<File>();
    if (imgFileList === undefined) return 0;

    for (let i = 0; i < imgFileList.length; i++) {
        if (imgFileList.item(i) === null) return 0;
        var img = new Image;
        img.onload = () =>
        {
            if(canvas) {
        ...
        img.src = URL.createObjectURL(imgFileList.item(i));
    } 
